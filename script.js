...
(() => {
  ...
  const LOCAL_MATHJAX_SRC = "mathjax/es5/tex-mml-chtml.js";
  ...
  const ui = {
    ...
    optionAddDockBtn: document.getElementById("optionAddDockBtn"),
  ...
  function bind() {
    ...
    ui.optionAddDockBtn.addEventListener("click", addOption);
    ...
    [ui.importReportModal, ui.preflightModal, ui.shortcutsModal].forEach((modal) => {
  ...
  function ensureMathJax() {
    ...
    if (document.querySelector("script[data-mathjax-loader='1']")) {
      return;
    }

    const script = document.createElement("script");
    script.defer = true;
    script.src = LOCAL_MATHJAX_SRC;
  ...
  function onGlobalKeydown(event) {
    ...
    if (event.key === "Escape" && document.activeElement === ui.mathSearchInput && ui.mathSearchInput.value) {
      event.preventDefault();
  ...
  function resolveActiveEditorInput() {
    if (editor.activeInput && ui.form.contains(editor.activeInput)) {
      return editor.activeInput;
    }
    if (document.activeElement instanceof HTMLTextAreaElement && ui.form.contains(document.activeElement)) {
  ...
  function openEditor() {
    ...
    window.requestAnimationFrame(() => {
      const target = editor.activeInput && ui.form.contains(editor.activeInput) ? editor.activeInput : ui.questionInput;
      target.focus();
    });
  }
  ...
  function renderEditor() {
    ...
    editor.draft.options.forEach((optionText, index) => {
      ...
      input.addEventListener("input", (event) => {
        ...
        markEditorTyping();
        scheduleOptionPreview(preview, event.target.value, hint);
        syncDirty();
        scheduleHistoryFromDraft();
      });
  ...
  function moveActiveOption(direction) {
    ...
    const currentValue = editor.draft.options[currentIndex];
    editor.draft.options[currentIndex] = editor.draft.options[targetIndex];
    editor.draft.options[targetIndex] = currentValue;

    if (editor.draft.correctIndex === currentIndex) {
      editor.draft.correctIndex = targetIndex;
    } else if (editor.draft.correctIndex === targetIndex) {
      editor.draft.correctIndex = currentIndex;
    }
  ...
  function refreshOptionToolbar() {
    ...
    ui.optionAddDockBtn.disabled = optionCount >= MAX_OPTIONS;
  ...
  function exportAnswerKeyJson() {
    ...
    download("answer-key.json", JSON.stringify(key, null, 2), "application/json");
    status("Answer key exported.", "success");
  }
  ...
  function createExamFileAfterPreflight() {
    ...
    const html = buildExamHtml(examTitle, examData, examConfig);
...
<body>
  ...
  function openPrintWindow(content) {
    ...
    if (!win) {
      status("Popup blocked. Allow popups to print.", "error");
      return;
    }
    win.document.open();
...
<tr><td>Exam Generation</td><td class="ok">Use Create Exam File button</td></tr>
  ...
  function migrateStoredPayload(payload) {
    ...
    return {
      ...
      examConfig: payload.examConfig && typeof payload.examConfig === "object"
        ? payload.examConfig
        : { preset: "quick", security: { fullscreen: false, navWarn: true, token: true } },
  ...
  function buildExamHtml(examTitle, questionSet, config) {
    ...
    const safeLocalMathSrc = JSON.stringify("./mathjax/es5/tex-mml-chtml.js");
...
<head>
  ...
  <style>
    :root{color-scheme:light;--bg:#f3f6fb;--surface:#fff;--soft:#f8faff;--text:#172033;--muted:#5b6880;--border:#d7e0ee;--primary:#1d4ed8;--primaryStrong:#1e40af;--ok:#0f766e;--bad:#b42318;--radius:14px;--shadow:0 12px 28px rgba(15,23,42,.1);--speed:300ms ease}
    :root[data-theme="dark"]{color-scheme:dark;--bg:#0b1220;--surface:#101a2d;--soft:#15213a;--text:#e6eefc;--muted:#9ab0d4;--border:#2a3b5a;--primary:#4f8cff;--primaryStrong:#2f6fe9;--ok:#18a295;--bad:#d3453a;--shadow:0 16px 34px rgba(0,0,0,.4)}
  ...
  ${scriptOpenTag}
    (() => {
      ...
      function renderExam() {
        ...
        ui.progress.textContent = "Questions: " + selectedQuestions.length + " | Marks: " + marksTotal;
      ...
      function evaluate() {
        ...
        return { correctCount, total, percent, elapsed, details, earnedMarks, totalMarks };
      }
      ...
      function renderResult(result, mode) {
        ...
        notes.push("Marks: " + result.earnedMarks + "/" + result.totalMarks + ".");
        if (attemptToken) {
